...
def imshow(tensor, title=None):
    image = tensor.cpu().clone()  # move tensor back to CPU
    image = image.squeeze(0)  # remove an axis
...
def gram_matrix(input):
    ...
    features = input.view(a * b, c * d) # reshaping a view onto the array
...
class Normalization(nn.Module):
    def __init__(self, mean, std):
        ...
        self.mean = torch.tensor(mean).view(-1, 1, 1)  # -1 --> infer dimension to match orig # elem
...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std,
    ...
    for layer in cnn.children():
        ...
        if name in content_layers:
            target = model(content_img).detach()  # don't compute gradient for this (I think)
    ...
    for i in range(len(model) - 1, -1, -1):  # find index of last loss module
        if isinstance(model[i], ContentLoss) or isinstance(model[i], StyleLoss):
            break

    model = model[:(i+1)]  # truncate model there
...
def run_style_transfer(cnn, normalization_mean, normalization_std,
    ...
    while run[0] <= num_steps:
        def closure():  # passing in closure to allow recomputing model (function) mult times
